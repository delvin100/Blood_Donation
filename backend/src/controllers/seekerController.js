...
exports.getDonors = async (req, res) => {
    try {
        ...
        let sql = 'SELECT id, username, full_name, phone, blood_type, state, district, city FROM donors WHERE availability = "Available"';
        const params = [];
...
exports.addSeeker = async (req, res) => {
    try {
        const { full_name, email, phone, blood_type, required_by, state, district, city } = req.body;
        const [result] = await pool.query(
            `INSERT INTO seekers (full_name, email, phone, blood_type, required_by, country, state, district, city, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
            [full_name, email, phone, blood_type, required_by, 'India', state, district, city]
        );
        res.json({ id: result.insertId, message: 'Request saved successfully' });
    } catch (err) {
        console.error("Error in addSeeker:", err);
        res.status(500).json({ error: 'Server error' });
    }
};
