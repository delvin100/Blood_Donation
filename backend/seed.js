...
// Simple random data generators since we might not have faker installed
...
const dbConfig = {
    ...
    database: 'ebloodbank', // Ensure this matches your DB name
...
async function seed() {
    ...
    try {
        ...
        // 1. Clear existing data (optional, but good for "hardcore" reset)
        console.log('🧹 Clearing old data...');
        // Disable FK checks to allow truncation
        await connection.query('SET FOREIGN_KEY_CHECKS = 0');
        const tables = ['donors', 'organizations', 'blood_inventory', 'emergency_requests', 'medical_reports', 'org_members', 'notifications'];
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const donor = {
                ...
                state: 'StateName', // Placeholder
                city: getRandom(cities),
                dob: '1995-01-01', // reliable date
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const [res] = await connection.query(
                ...
                [
                    orgName,
                    `contact${i}@${city.toLowerCase().replace(' ', '')}org.com`,
                    generatePhone(),
                    passwordHash,
        ...
        for (const donor of donors) {
            ...
            for (const org of selectedOrgs) {
                await connection.query(
                    ...
                    [org.id, donor.id, 'Member']
                );
            }
        }

        // 6. Create Medical Reports (Donation History)
        console.log('📄 Generating Medical Reports...');
        for (let i = 0; i < 15; i++) {
            const donor = getRandom(donors);
            const org = getRandom(orgs);
            const date = new Date();
            date.setDate(date.getDate() - getRandomInt(1, 365)); // past year
            ...
            await connection.query(
                ...
                [
                    donor.id,
                    org.id,
                    (12 + Math.random() * 4).toFixed(1), // HB 12-16
                    '120/80',
                    getRandomInt(50, 90),
                    1,
                    donor.blood_type,
                    date,
                    'Routine donation. All vitals normal.'
                ]
            );
        }
