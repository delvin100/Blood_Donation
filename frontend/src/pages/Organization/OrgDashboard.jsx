...
export default function OrgDashboard() {
    ...
    const downloadHistoryCSV = () => {
        ...
        const headers = ["Activity,Description,Date,Time"];
        const rows = history.map(h => {
            const date = new Date(h.created_at);
            return `"${(h.action_type || '').replace(/_/g, ' ')}","${h.description || ''}",${date.toLocaleDateString('en-GB')},${date.toLocaleTimeString()}`;
        });
        ...
        link.setAttribute("download", `org_logs_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    const downloadLogsPDF = async () => {
        ...
        setTimeout(async () => {
            try {
                ...
                const template = document.getElementById('logs-report-template');
                const pdf = new jsPDF('p', 'mm', 'a4');
                const pdfWidth = pdf.internal.pageSize.getWidth();

                // Chunk history into groups of 7
                const chunkSize = 7;
                const chunks = [];
                ...
                for (let i = 0; i < chunks.length; i++) {
                    // Update state with current chunk and page meta
                    setPdfExportChunk(chunks[i]);
                    setPdfPageMeta({
                        isFirstPage: i === 0,
                        isLastPage: i === chunks.length - 1
                    });

                    // Small delay to allow React to re-render the template
                ...
                pdf.save(`Activity_Logs_${new Date().toISOString().split('T')[0]}.pdf`);
                toast.success('Activity Logs Exported as PDF (7 per page)');

                // Clear the chunk and meta state
                setPdfExportChunk([]);
                setPdfPageMeta({ isFirstPage: false, isLastPage: false });
    ...
    const fetchDonorReportsForVerification = async (donorId) => {
        ...
        try {
            ...
            setVerificationHistory(res.data);
        } catch (err) {
    ...
    const handleVerifyDonation = async () => {
        ...
        const donorId = verificationResult.donor_id || verificationResult.id;

        // Fetch historical reports for this donor
        fetchDonorReportsForVerification(donorId);

        // Pre-fill blood group and rh factor
        setVerificationReportForm(prev => ({
            ...prev,
            blood_group: verificationResult.blood_type || 'O+',
            rh_factor: verificationResult.blood_type?.includes('+') ? 'Positive' : (verificationResult.blood_type?.includes('-') ? 'Negative' : 'Positive'),
            units_donated: '1'
        }));
        setVerificationFormErrors({}); // Reset errors from previous sessions
        setShowVerificationReportModal(true);
    };
    ...
    const validateVerificationReport = () => {
        ...
        if (!verificationReportForm.hb_level || verificationReportForm.hb_level < 5 || verificationReportForm.hb_level > 25)
            errors.hb_level = "Invalid Hb (5-25 g/dL)";
        if (!verificationReportForm.blood_pressure || !bpRegex.test(verificationReportForm.blood_pressure))
            errors.blood_pressure = "Use format 120/80";
        if (!verificationReportForm.pulse_rate || verificationReportForm.pulse_rate < 40 || verificationReportForm.pulse_rate > 200)
            errors.pulse_rate = "Invalid Pulse (40-200)";
        if (!verificationReportForm.temperature || verificationReportForm.temperature < 35 || verificationReportForm.temperature > 42)
            errors.temperature = "Invalid Temp (35-42°C)";
        if (!verificationReportForm.weight || verificationReportForm.weight < 30 || verificationReportForm.weight > 250)
            errors.weight = "Invalid Weight (30-250kg)";
        if (verificationReportForm.units_donated === '' || verificationReportForm.units_donated < 0 || verificationReportForm.units_donated > 1.0)
            errors.units_donated = "Units (0.0 or 1.0)";

        setVerificationFormErrors(errors);
        return Object.keys(errors).length === 0;
    };

    const handleFinalizeVerification = async (e) => {
        if (e) e.preventDefault();
        if (!validateVerificationReport()) {
            return toast.error("Please correct the clinical indicators");
        }

        const donorId = verificationResult.donor_id || verificationResult.id;
        const finalizeToast = toast.loading("Finalizing donation & recording clinical data...");
        try {
            const token = getAuthToken();

            // 1. Submit Medical Report
            await axios.post(`/api/organization/member/${donorId}/reports`, { ...verificationReportForm, isDonation: true }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            ...
            await axios.post('/api/organization/verify',
                { donor_id: donorId, notes: 'Verified with clinical report' },
                { headers: { Authorization: `Bearer ${token}` } }
            );

            toast.success("Donation Verified & Clinical Data Logged", { id: finalizeToast });
            setShowVerificationReportModal(false);
            setVerificationResult(null);
            setVerificationFormErrors({}); // Clear errors on success
            fetchStats();
            fetchHistory();
            fetchActivity();
        } catch (err) {
            toast.error(err.response?.data?.error || "Verification failed", { id: finalizeToast });
        }
    };
    ...
    const generateClinicalPDF = async (report, donor) => {
        setIsGeneratingPDF(true);
        setActiveReportForPDF(report);
        setSelectedDonor(donor); // Store donor context for template

        setTimeout(async () => {
            try {
                const html2canvas = (await import('html2canvas')).default;
                const template = document.getElementById('org-report-template');
                ...
                pdf.save(`Medical_Report_${report.id || 'RC'}_${donor.full_name.replace(/\s+/g, '_')}.pdf`);
    ...
    const generateVerificationPDF = async (report) => {
        setIsGeneratingPDF(true);
        setActiveReportForPDF(report);
        // Map verificationResult to selectedDonor for the template
        setSelectedDonor(verificationResult);
        ...
        setTimeout(async () => {
            try {
                ...
                pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
                pdf.save(`Medical_Record_${report.id || 'REP'}_Verified.pdf`);
                toast.success("Medical Report Downloaded");
            } catch (err) {
                console.error("PDF Export failed", err);
                toast.error("Failed to generate professional PDF");
    ...
    return (
        <div className="flex h-screen bg-gray-50 font-sans text-gray-900">
            ...
            <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${sidebarOpen ? 'ml-72' : 'ml-24'}`}>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h2 className="text-xl font-black text-gray-900">
                                ...
                                {activeTab === 'verification' && 'Add Member to Organisation'}
                    ...
                    {activeTab === 'home' && (
                        <div className="max-w-7xl mx-auto space-y-10">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center py-10">
                                    <div className="space-y-6 max-w-4xl">
                                        ...
                                        <div className="flex flex-wrap gap-6 justify-center pt-4">
                                            <button onClick={() => setActiveTab('verification')} className="px-10 py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 flex items-center gap-3 active:scale-95 text-lg">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <div className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8 flex flex-col group/card relative overflow-hidden">
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-red-50 rounded-full blur-[60px] opacity-20 -mr-16 -mt-16 transition-all group-hover/card:scale-150"></div>
                                    <div className="space-y-2 relative z-10">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Geographic Reach</h3>
                                                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Active Member Localities</p>
                                            </div>
                                            <div className="w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 group-hover/card:text-red-500 group-hover/card:bg-red-50 transition-all duration-500">
                                                <i className="fas fa-map-marked-alt text-xl"></i>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="space-y-8 flex-1 py-6 relative z-10 max-h-[450px] overflow-y-auto pr-4 scrollbar-thin">
                                        {geoReach.length > 0 ? geoReach.map((geo, i) => (
                                            <div key={i} className="group/item border-b border-gray-50 pb-6 last:border-0 last:pb-0">
                                                <div className="flex justify-between items-center">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover/item:bg-red-500 group-hover/item:text-white transition-all duration-300 shadow-sm">
                                                            <i className="fas fa-location-dot text-sm"></i>
                                                        </div>
                                                        <div>
                                                            <span className="text-sm font-black text-gray-800 block group-hover/item:text-gray-900 transition-colors uppercase tracking-tight">{geo.city}</span>
                                                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Primary Reach</span>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <span className="text-xl font-black text-gray-900 block tabular-nums leading-none mb-1">{geo.count}</span>
                                                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Donors</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )) : (
                                            <div className="h-full flex flex-col items-center justify-center text-center py-12">
                                                <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center text-gray-200 mb-6 relative">
                                                    <i className="fas fa-globe-asia text-4xl animate-pulse"></i>
                                                    <div className="absolute inset-0 border-2 border-dashed border-gray-100 rounded-full animate-[spin_10s_linear_infinite]"></div>
                                                </div>
                                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">Analyzing Footprint</p>
                                                <p className="text-xs font-bold text-gray-300 max-w-[200px] leading-relaxed">Localities will emerge as your donor network grows.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="max-w-6xl mx-auto">
                            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                                <div className="p-10 border-b border-gray-50 flex items-center justify-between">
                                    <div>
                                        <h3 className="text-2xl font-black text-gray-900 mb-2">Organization Logs</h3>
                                        <p className="text-gray-500 font-medium">Complete audit trail of all organization activities.</p>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <button
                                            onClick={downloadHistoryCSV}
                                            className="w-12 h-12 bg-blue-50 hover:bg-blue-100 rounded-2xl flex items-center justify-center text-blue-500 hover:text-blue-700 transition-all border border-blue-100 shadow-sm"
                                            title="Export CSV"
                                        >
                                        ...
                                        <button
                                            ...
                                            title="Export PDF"
                                        >
                                            {isExportingPDF ? <i className="fas fa-circle-notch fa-spin"></i> : <i className="fas fa-file-pdf"></i>}
                                        </button>
                                    </div>
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="bg-gray-50/50 border-b border-gray-100">
                                                    <th className="p-6 text-xs font-black text-gray-400 uppercase tracking-widest">Activity</th>
                            ...
                            <div className="max-w-2xl mx-auto">
                                <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                                    <div className="p-10 border-b border-gray-50">
                                        <h3 className="text-2xl font-black text-gray-900 mb-2">Add Member to Organisation</h3>
                                        <p className="text-gray-500 font-medium">Search for a donor by email or phone to add them to your organisation.</p>
                                    </div>
                                    <div className="p-10">
                                        <DonorSearch
                                            verifying={verifying}
                                        ...
                                        {verificationResult ? (
                                            <div className="bg-blue-50/50 rounded-3xl p-8 border border-blue-100">
                                                <div className="flex items-center gap-6 mb-8">
                                                    <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 text-3xl font-black">
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    <button
                                                        ...
                                                        className="w-full bg-blue-600 text-white font-black py-5 rounded-[1.5rem] hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 active:scale-95 flex items-center justify-center gap-3"
                                                    >
                                                        <i className="fas fa-user-plus"></i>
                                                        Add to Organisation
                                                    </button>
                ...
                {/* --- CLINICAL VERIFICATION MODAL (REDESIGNED) --- */}
                {
                    showVerificationReportModal && (
                        <div className="fixed inset-0 z-[250] bg-[#f8fafc] flex flex-col animate-in fade-in zoom-in-95 duration-500 overflow-hidden">
                            {/* Header Section */}
                            <div className="px-8 lg:px-12 py-6 bg-white border-b border-slate-200 flex items-center justify-between sticky top-0 z-30 shadow-sm">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="space-y-0.5">
                                        <div className="flex items-center gap-3">
                                            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Clinical Verification Entry</h3>
                                            <span className="px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-black uppercase tracking-wider border border-red-100/50">Urgent Verification</span>
                                        </div>
                                        <div className="flex items-center gap-3 text-slate-500">
                                            <p className="text-sm font-semibold">{verificationResult?.full_name}</p>
                                            <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                                            <p className="text-xs font-medium uppercase tracking-widest text-red-600">{verificationResult?.donor_tag}</p>
                                        </div>
                                    </div>
                                </div>

                                <button
                                    onClick={() => setShowVerificationReportModal(false)}
                                        ...
                                        <div className="xl:col-span-5 space-y-8">
                                            <div className="flex items-center justify-between px-2">
                                                ...
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white border border-slate-200 px-3 py-1 rounded-lg">
                                                    {verificationHistory.length} Established Records
                                                </p>
                                            </div>
                                            ...
                                            {verificationHistoryLoading ? (
                                                ...
                                                <div className="space-y-6">
                                                    {verificationHistory.map((report, idx) => (
                                                        <div key={idx} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-300">
                                                            <div className="flex justify-between items-start mb-8">
                                                                ...
                                                                <div className="flex flex-col items-end gap-3">
                                                                    <span className="px-4 py-1.5 bg-red-50 text-red-700 rounded-xl text-sm font-black border border-red-100/50">
                                                                        {report.blood_group} {report.rh_factor}
                                                                    </span>
                                                                    <button
                                                                        onClick={() => generateVerificationPDF(report)}
                                                                        className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 hover:text-red-600 transition-colors"
                                                                    >
                                                                ...
                                                                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                                                    <div className="flex items-center gap-2 mb-6">
                                                                        <div className="w-7 h-7 bg-red-100 rounded-lg flex items-center justify-center">
                                                                            <i className="fas fa-heartbeat text-red-600 text-xs"></i>
                                                                        </div>
                                                                        ...
                                                                        {[
                                                                            ...
                                                                            <div key={i} className="flex flex-col gap-1">
                                                                                ...
                                                                                <div className="flex items-baseline gap-1.5">
                                                                                    <span className={`text-base font-black ${item.highlight ? "text-red-600" : "text-slate-900"}`}>{item.value}</span>
                                                            ...
                                                            {report.notes && (
                                                                <div className="mt-8 p-5 bg-white border border-slate-200 rounded-2xl relative overflow-hidden group">
                                                                    ...
                                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                                                        <i className="fas fa-comment-medical text-red-500"></i>
                                        ...
                                        <div className="xl:col-span-7">
                                            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 lg:p-12 shadow-sm sticky top-32">
                                                <div className="space-y-8">
                                                    ...
                                                    <form onSubmit={handleFinalizeVerification} className="space-y-10">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                                            <div className="space-y-2">
                                                                ...
                                                                <input
                                                                    type="number" step="0.1" min="5" max="25" value={verificationReportForm.hb_level}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, hb_level: e.target.value })}
                                                                    className={`w-full bg-slate-50 border-2 ${verificationFormErrors.hb_level ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-100'} rounded-2xl px-6 py-4 font-bold text-slate-900 focus:bg-white focus:border-indigo-600 outline-none transition-all`}
                                                                    placeholder="g/dL"
                                                                />
                                                                {verificationFormErrors.hb_level && <p className="text-[11px] text-red-600 font-bold mt-1.5 ml-1">{verificationFormErrors.hb_level}</p>}
                                                            </div>
                                                            <div className="space-y-2">
                                                                ...
                                                                <input
                                                                    ...
                                                                    value={verificationReportForm.blood_pressure}
                                                                    onChange={e => {
                                                                        const val = e.target.value.replace(/[^0-9/]/g, '');
                                                                        setVerificationReportForm({ ...verificationReportForm, blood_pressure: val });
                                                                    }}
                                                                    className={`w-full bg-slate-50 border-2 ${verificationFormErrors.blood_pressure ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-100'} rounded-2xl px-6 py-4 font-bold text-slate-900 focus:bg-white focus:border-indigo-600 outline-none transition-all`}
                                                                    placeholder="120/80"
                                                                />
                                                                {verificationFormErrors.blood_pressure && <p className="text-[11px] text-red-600 font-bold mt-1.5 ml-1">{verificationFormErrors.blood_pressure}</p>}
                                                            </div>
                                                            <div className="space-y-2">
                                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Units (Blood)</label>
                                                                <input
                                                                    type="number" step="1" min="0" max="1" value={verificationReportForm.units_donated}
                                                                    onKeyDown={(e) => {
                                                                    ...
                                                                    onChange={e => {
                                                                        ...
                                                                        if (val === '0' || val === '1' || val === '') {
                                                                            setVerificationReportForm({ ...verificationReportForm, units_donated: val });
                                                                        }
                                                                    }}
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <input
                                                                    type="number" min="30" max="250" value={verificationReportForm.weight}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, weight: e.target.value })}
                                                                    className={`w-full bg-slate-50 border-2 ${verificationFormErrors.weight ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-100'} rounded-2xl px-6 py-4 font-bold text-slate-900 focus:bg-white focus:border-indigo-600 outline-none transition-all`}
                                                                    placeholder="kg"
                                                                />
                                                                {verificationFormErrors.weight && <p className="text-[11px] text-red-600 font-bold mt-1.5 ml-1">{verificationFormErrors.weight}</p>}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                                                            <div className="space-y-2">
                                                                ...
                                                                <input
                                                                    type="number" min="40" max="200" value={verificationReportForm.pulse_rate}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, pulse_rate: e.target.value })}
                                                                    className={`w-full bg-slate-50 border-2 ${verificationFormErrors.pulse_rate ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-100'} rounded-2xl px-5 py-4 font-bold text-slate-900 focus:bg-white focus:border-indigo-600 outline-none transition-all`}
                                                                    placeholder="BPM"
                                                                />
                                                                {verificationFormErrors.pulse_rate && <p className="text-[11px] text-red-600 font-bold mt-1.5 ml-1">{verificationFormErrors.pulse_rate}</p>}
                                                            </div>
                                                            <div className="space-y-2">
                                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Temperature</label>
                                                                <input
                                                                    type="number" step="0.1" min="35" max="42" value={verificationReportForm.temperature}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, temperature: e.target.value })}
                                                                    className={`w-full bg-slate-50 border-2 ${verificationFormErrors.temperature ? 'border-red-400 ring-4 ring-red-50' : 'border-slate-100'} rounded-2xl px-5 py-4 font-bold text-slate-900 focus:bg-white focus:border-indigo-600 outline-none transition-all`}
                                                                    placeholder="°C"
                                                                />
                                                                {verificationFormErrors.temperature && <p className="text-[11px] text-red-600 font-bold mt-1.5 ml-1">{verificationFormErrors.temperature}</p>}
                                                            </div>
                                                            <div className="space-y-2">
                                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Blood Group</label>
                                                                <select
                                                                    disabled
                                                                    value={verificationReportForm.blood_group}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, blood_group: e.target.value })}
                                                            ...
                                                            <div className="space-y-2">
                                                                ...
                                                                <select
                                                                    disabled
                                                                    value={verificationReportForm.rh_factor}
                                                                    onChange={e => setVerificationReportForm({ ...verificationReportForm, rh_factor: e.target.value })}
                                                                    className="w-full bg-slate-100 border-2 border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-500 outline-none transition-all cursor-not-allowed appearance-none"
                                                                >
                                                            ...
                                                            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                                                                {['hiv_status', 'hepatitis_b', 'hepatitis_c', 'syphilis', 'malaria'].map(test => (
                                                                    <div key={test} className="space-y-2">
                                                                        ...
                                                                        <select
                                                                            value={verificationReportForm[test]}
                                                                            onChange={e => setVerificationReportForm({ ...verificationReportForm, [test]: e.target.value })}
                                                                            className={`w-full ${verificationReportForm[test] === 'Negative' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100'} border rounded-xl px-4 py-3 font-bold text-[11px] outline-none transition-all cursor-pointer`}
                                                                        >
                                                                            <option>Negative</option>
                                                                            <option>Positive</option>
                                                                        </select>
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            <textarea
                                                                value={verificationReportForm.notes}
                                                                onChange={e => setVerificationReportForm({ ...verificationReportForm, notes: e.target.value })}
                                                        ...
                                                        <div className="flex gap-6">
                                                            <button
                                                                type="button"
                                                                onClick={() => setShowVerificationReportModal(false)}
                                                                className="flex-1 py-6 bg-slate-100 text-slate-900 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-200"
                                                            >
                                                                Cancel Verification
                                                            </button>
                                                            <button
                                                                type="submit"
                                                                className="flex-[2] py-6 bg-red-600 text-white rounded-[2rem] font-black text-lg hover:bg-red-700 hover:shadow-2xl hover:shadow-red-500/20 transition-all flex items-center justify-center gap-4 group shadow-xl"
                                                            >
                                                                <span>Complete Verification</span>
                                                                <i className="fas fa-shield-check group-hover:rotate-12 transition-transform"></i>
                                                            </button>
                                                        </div>
                                                    </form>
                                                </div>
                ...
                <div className="fixed -left-[9999px] top-0">
                    <div id="org-report-template" className="bg-white p-12 w-[800px]">
                        ...
                        <div className="grid grid-cols-2 gap-12 mb-12">
                            <div>
                                ...
                                <div className="space-y-2">
                                    <p className="text-xl font-black text-gray-800">{selectedDonor?.full_name}</p>
                                    <p className="text-gray-600 font-medium">{selectedDonor?.email}</p>
                                    <p className="text-gray-600 font-medium">{selectedDonor?.phone || 'N/A'}</p>
                                    <div className="pt-2">
                                        <span style={{
                                            color: '#334155',
                                            fontWeight: '800',
                            ...
                            <div className="text-right">
                                ...
                                <div className="space-y-2">
                                    <p className="text-xl font-black text-gray-800">{orgDetails?.name}</p>
                                    <p className="text-gray-600 font-medium">{orgDetails?.email || 'Authorized Facility'}</p>
                                    <p className="text-sm text-gray-500">{orgDetails?.address || 'Verified Center'}</p>
                                    <p className="text-gray-600 font-medium">Date: {activeReportForPDF?.test_date ? new Date(activeReportForPDF.test_date).toLocaleDateString('en-GB') : 'N/A'}</p>
                ...
                {/* HIDDEN LOGS PDF TEMPLATE */}

                <div className="fixed -left-[9999px] top-0">
                    <div id="logs-report-template" className="bg-white p-16 pb-32 w-[1000px]">

                        <div className="border-b-8 border-blue-600 pb-10 mb-12 flex justify-between items-end">
                            <div>
                                <h1 className="text-5xl font-black text-slate-900 mb-4 tracking-tighter">Organization Activity Logs</h1>
                                <div className="flex items-center gap-4 text-slate-500 font-bold">
                                    <p className="text-base italic">Generated on {new Date().toLocaleDateString('en-GB')} {new Date().toLocaleTimeString()}</p>
                                </div>

                            </div>
                            <div className="text-right">
                                <div className="text-3xl font-black text-blue-600 mb-2">eBloodBank</div>
                                <p className="text-xs text-slate-400 font-black uppercase tracking-[0.3em]">Lifecycle Integrity Network</p>
                            </div>
                        </div>

                        {pdfPageMeta.isFirstPage && (
                            <div className="grid grid-cols-2 gap-16 mb-16">
                                <div className="bg-slate-50 rounded-[2.5rem] p-10 border border-slate-100">
                                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Subject Organization</h3>
                                    <div className="space-y-3">
                                        <p className="text-3xl font-black text-slate-900 leading-tight">{orgDetails?.name || 'Loading...'}</p>
                                        <div className="space-y-1">
                                            <p className="text-slate-600 font-bold flex items-center gap-3">
                                                <i className="fas fa-envelope text-blue-500 w-5"></i> {orgDetails?.email}
                                            </p>
                                            <p className="text-slate-600 font-bold flex items-center gap-3">
                                                <i className="fas fa-phone text-blue-500 w-5"></i> {orgDetails?.phone || 'N/A'}
                                            </p>
                                            <p className="text-slate-600 font-bold flex items-center gap-3">
                                                <i className="fas fa-id-badge text-blue-500 w-5"></i> {orgDetails?.license_number}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-900">
                                        <th className="p-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-800">Activity Type</th>
                                        <th className="p-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-800">Description</th>
                                        <th className="p-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-800">Date</th>
                                        <th className="p-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-800">Time</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {pdfExportChunk.map((item, idx) => (
                                        ...
                                        <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}>
                                            ...
                                            <td className="p-8">
                                                <div className="text-sm font-bold text-slate-800 leading-relaxed max-w-[400px]">
                                                    {item.description}
                                                </div>
                                            </td>
                                            <td className="p-8">
                        ...
                        <div className="mt-12 pt-10 border-t border-slate-100 flex justify-between items-end">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] text-slate-300 font-bold italic">© {new Date().getFullYear()} eBloodBank Life Saving Network. All rights reserved.</p>
                            </div>

                            {pdfPageMeta.isLastPage && (
                                <div className="text-center w-64 pt-8">
                                    <div className="h-0.5 bg-slate-900 mb-4 scale-x-75"></div>
                                    <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em]">System Administrator</p>
                                </div>
                            )}
