...
export default function OrgDashboard() {
    ...
    const handleVerifyDonation = async () => {
        ...
        fetchDonorReportsForVerification(verificationResult.id);

        // Pre-fill blood group and rh factor
        setVerificationReportForm(prev => ({
            ...prev,
            blood_group: verificationResult.blood_type?.replace(/[+-]/g, '') || 'O+',
    ...
    const validateVerificationReport = () => {
        ...
        if (!verificationReportForm.units_donated || verificationReportForm.units_donated < 0.1 || verificationReportForm.units_donated > 5.0)
    ...
    const handleFinalizeVerification = async (e) => {
        ...
        try {
            ...
            await axios.post(`/api/organization/member/${verificationResult.id}/reports`, verificationReportForm, {
                headers: { Authorization: `Bearer ${token}` }
            });

            // 2. Submit Verification
            await axios.post('/api/organization/verify',
                { donor_id: verificationResult.id, notes: 'Verified with clinical report' },
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const generateVerificationPDF = (report) => {
        const doc = new jsPDF();
        ...
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.text(`Authenticated Digital Health Record | ${verificationResult?.donor_tag || 'Verified Member'}`, 20, 35);
        doc.text(`Ref: ${verificationResult?.donor_tag || 'DH-00' + (verificationResult?.id)}`, 190, 35, { align: "right" });
        ...
        doc.setTextColor(15, 23, 42);
        doc.setFontSize(12);
        doc.text(`Name: ${verificationResult?.full_name}`, 25, 80);
        doc.text(`Donor ID: ${verificationResult?.donor_tag || '#DH-00' + (verificationResult?.id)}`, 25, 88);
        ...
        doc.save(`Medical_Report_${verificationResult?.full_name.replace(' ', '_')}_#${report.id}.pdf`);
        toast.success("Medical Report Downloaded");
    };
                                                ...
                                                <form onSubmit={handleFinalizeVerification} className="space-y-10">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            <input
                                                                type="number" step="0.1" min="0.1" max="5.0" value={verificationReportForm.units_donated}
                                                                onChange={e => setVerificationReportForm({ ...verificationReportForm, units_donated: e.target.value })}
