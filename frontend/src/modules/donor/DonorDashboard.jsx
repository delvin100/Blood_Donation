...
const Dashboard = () => {
  ...
  const generateReportPDF = (report) => {
    ...
    doc.text(`Generated by eBloodBank | Facility: ${report.org_name || 'Verified Center'}`, 20, 35);
    ...
    doc.setTextColor(50);
    doc.setFontSize(11);
    doc.text(`Donor Name: ${user.full_name}`, 25, 60);
    doc.text(`Blood Group: ${report.blood_group} ${report.rh_factor}`, 25, 70);
    doc.text(`Report ID: #${report.id}`, 120, 60);
    doc.text(`Date: ${dateStr}`, 120, 70);

    // Vitals Section
    doc.setFontSize(14);
    doc.setTextColor(15, 23, 42);
    doc.text("Clinical Vitals", 20, 100);
    ...
    let y = 115;
    const vitals = [
      { l: "Hemoglobin", v: `${report.hb_level} g/dL` },
      { l: "Blood Pressure", v: report.blood_pressure },
      { l: "Pulse Rate", v: `${report.pulse_rate} BPM` },
      { l: "Temperature", v: `${report.temperature} °C` },
      { l: "Weight", v: `${report.weight} kg` },
      { l: "Donated Units", v: report.units_donated || '1.0' }
    ];

    vitals.forEach((item, i) => {
      if (i % 2 === 0 && i !== 0) y += 15;
      const x = i % 2 === 0 ? 20 : 110;
      doc.text(`${item.l}:`, x, y);
      doc.setFont("helvetica", "bold");
      doc.text(item.v, x + 40, y);
      doc.setFont("helvetica", "normal");
    });
    ...
    doc.setFontSize(14);
    doc.setTextColor(15, 23, 42);
    doc.text("Infectious Disease Screening", 20, y);
    doc.line(20, y + 2, 90, y + 2);

    y += 15;
    doc.setFontSize(11);
    const tests = [
      { l: "HIV", v: report.hiv_status },
      { l: "Hepatitis B", v: report.hepatitis_b },
      { l: "Hepatitis C", v: report.hepatitis_c },
      { l: "Syphilis", v: report.syphilis },
      { l: "Malaria", v: report.malaria }
    ];

    tests.forEach(t => {
      doc.setTextColor(80);
      doc.text(t.l, 20, y);
      if (t.v === 'Negative') doc.setTextColor(0, 150, 0); // Green
      else doc.setTextColor(200, 0, 0); // Red
      doc.text(t.v.toUpperCase(), 160, y, { align: 'right' });
    ...
    if (report.notes) {
      ...
      doc.setTextColor(80);
      doc.setFontSize(10);
      doc.text(doc.splitTextToSize(report.notes, 170), 20, y + 7);
    }

    doc.save(`Medical_Report_${dateStr}.pdf`);
  };
